...
Celem laboratorium było przeanalizowanie logów uwierzytelniania systemu Linux ooraz wykrycie przejęcia konta przez SSH.
...
- nadanie nowemu kontu uprawnień administratora
...
W systemach Ubuntu i Debian zdarzenia uwierzytelniania często znajdują się w: `/var/log/auth.log`

W systemach takich jak RHEL lub CentOS podobne informacje mogą być zapisywane w: `/var/log/secure`

W  systemach używających `systemd` logi można również analizować za pomocą: `journalctl`

# Windows a Linux

W systemie Windows zdarzenia są często identyfikowane przez EventID:

4624 -> udane logowanie
4625 -> nieudane logowanie

W linuxie podobne informacje mogą występować jako tekst:
Failed password, Accepted password, session opened, sudo, useradd
...
konto, host, adres IP, czas, wynik logowania, dalsza aktywność
